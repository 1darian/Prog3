...
const rutaTurnos = require("./routes/turnos.route.js") // Nueva importación
...
class Server {
  ...
  middleware() {
    ...
    this.app.use(express.urlencoded({ extended: true })) // Agrega esta línea
  }

  rutas() {
    this.app.use("/api/v1/pacientes", rutaPacientes)
    this.app.use("/api/v1/turnos", rutaTurnos) // Nueva ruta
    this.app.use("/", home)
